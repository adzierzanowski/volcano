...
const char *get_content_type(const char *path) {
  ...
  char **spath = split_path(ppath, &spathsz, ".");
  ...
  split_path_free(spath, spathsz);

  return mime;
}

char *make_http_response(int status, const char *content_type, uint8_t *body) {
...
char *make_api_response(int status, const char *json_msg) {
  char body[SMALLBUFSZ] = {0};
  sprintf(body, "{\"status\": \"%s\", \"msg\": \"%s\"}", status == 200 ? "ok" : "error", json_msg);
  return make_http_response(status, "application/json", (uint8_t *) body);
}
...
char **split_path(char *path, size_t *outsz, const char *path_delim) {
  ...
  *outsz = 0;
  ...
  char *tok = strtok(path, path_delim);
  while (tok != NULL) {
...
char *dispatch_cmd(char **spath, size_t spathsz, char *body) {
  ...
  if (spathsz == 0) {
    return make_api_response(400, "Empty command.");
  }

  if (strcmp(spath[0], "color") == 0) {
    if (spathsz != 4) {
      return make_api_response(400, "color command needs 3 arguments.");
    }
    ...
    return make_api_response(status, rxbuf);

  } else if (strcmp(spath[0], "kcolor") == 0) {
    if (spathsz != 5) {
      return make_api_response(400, "color command needs 4 arguments.");
    }
    ...
    if (spathsz != 2) {
      return make_api_response(400, "mode command needs exactly one argument.");
    }
    ...
    if (spathsz != 2) {
      return make_api_response(400, "speed command needs exactly one argument.");
    }
    ...
    if (spathsz != 2) {
      return make_api_response(400, "brightness command needs exactly one argument.");
    }
    ...
    if (spathsz != 2) {
      return make_api_response(400, "dir command needs exactly one argument.");
    }
    ...
    if (spathsz != 2) {
      return make_api_response(400, "rainbow command needs exactly one argument.");
    }
    ...
    if (spathsz != 2) {
      return make_api_response(400, "rate command needs exactly 2 arguments.");
    }
    ...
    if (body[0] != '[') {
      return make_api_response(400, "Bad kmap format.");
    }
  ...
  return make_api_response(400, "Unknown command.");
}

char *handle_request_post(char *path, char *body) {
  size_t spathsz = 0;
  char **spath = split_path(path, &spathsz, "/");
...
char *handle_request_get(char *path) {
  ...
  if (strmatch(path,
    ...
    if (f == NULL) {
      return make_http_response(404, "text/plain", (uint8_t *) http_status_str(404));
    }
    char fbuf[BUFSZ] = {0};
    ...
    return make_http_response(200, get_content_type(fpath), (uint8_t *) fbuf);

  } else {
    return make_http_response(418, "text/plain", (uint8_t *) http_status_str(418));
  }
}
...
char *handle_request(char *method, char *path, char *body) {
  if (strcmp(method, "POST") == 0) {
    ...
    return make_http_response(405, "text/plain", (uint8_t *) http_status_str(405));
  }
}
