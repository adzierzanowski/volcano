...
char *vlc_http_make_api_response(
   int status, const char *json_msg, size_t *ressz);

// Returns an HTTP response based on
// `status` - HTTP status code
// `mime_type` - Content-Type header (text/plain by default)
// Saves the size of the response in `ressz`
...
char *vlc_srv_handle_request(
   char *method, char *path, char *body, size_t *ressz);
...
// Returns a string containing an HTTP response
...
char *vlc_srv_cmd_handle(
   char **spath, size_t spathsz, char *body, size_t *ressz);
